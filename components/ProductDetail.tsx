...
// Camera icon shown when a device has no image
...
export default function ProductDetail({ product, parts }: ProductDetailProps) {
  ...
  // ── Initialize cart count from the server ──────────────────────────────────
  ...
  // The part currently being hovered (used for the image tooltip)
  const hoveredPart = hoveredPartId
    ? filteredParts.find((p) => p._id === hoveredPartId)
    : null;
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-8">
              ...
              <div className="w-24 h-24 rounded-lg border border-slate-200 shadow-sm bg-slate-100 overflow-hidden flex items-center justify-center shrink-0">
                {product.imageUrl ? (
                  <img
                    ...
                    onError={(e) => {
                      // If the URL is broken, fall back to the placeholder icon
                ...
                {filteredParts.length > 0 ? (
                  filteredParts.map((part) => (
                    <tr
                      ...
                      // Track the hovered part so we can show an image tooltip
                      onMouseEnter={(e) => {
                        if (part.imageUrl) {
                      ...
                      onMouseMove={(e) => {
                        if (part.imageUrl) setTooltipPos({ x: e.clientX, y: e.clientY });
                      }}
      ...
      {/* Image tooltip — follows the cursor over rows that have an image */}
      {hoveredPart?.imageUrl && (
        <div
