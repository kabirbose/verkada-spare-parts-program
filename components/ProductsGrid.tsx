...
export default function ProductsGrid({
  ...
  const handleDeletePart = async (e: React.MouseEvent, id: string) => {
    ...
    } catch (err) { console.error("Error deleting spare part:", err); }
  };
    ...
    <>
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto items-center flex-wrap xl:flex-nowrap">
          {/* View Mode Toggle */}
          <div className="flex bg-slate-100 p-1 rounded-lg w-full sm:w-auto shrink-0 shadow-inner">
          ...
          <Link
            href="/orders"
            className="w-full sm:w-auto px-5 py-2 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 rounded-lg font-medium transition-colors shadow-sm text-center flex items-center justify-center gap-2 shrink-0"
          >
          ...
          <Link
            href="/cart"
            className="relative w-full sm:w-auto px-5 py-2 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 rounded-lg font-medium transition-colors shadow-sm text-center flex items-center justify-center gap-2 shrink-0"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <Link
            href="/admin"
            className="w-full sm:w-auto px-5 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-medium transition-colors shadow-sm text-center flex items-center justify-center gap-2 shrink-0"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {prods.map((product) => (
                  <Link
                    ...
                    key={product._id}
                    className="group bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md hover:border-blue-400 transition-all duration-200 flex flex-col relative"
                  >
      ...
      {viewMode === "parts" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredParts.map((part) => (
            <div key={part._id} className="group bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md hover:border-blue-400 transition-all duration-200 flex flex-col relative p-5">
              <CardActionButtons
